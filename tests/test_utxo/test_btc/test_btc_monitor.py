...
async def test_async_monitoring_mysql(btc_client_mysql: AioTxBTCClient):
    ...
    await btc_client_mysql.import_address(
        "tb1putn7zkfjr97xd77as09w5syy3x7xr5crp99wycjcxgdwrq3le9lse9fdkd"
    )
    await btc_client_mysql.import_address(
    ...
    assert "3f419799747dc83a33a84a8d58fe5e9265d9d1b9b122de6914eb849abb04d9ae" in [
        tx["txid"] for tx in transactions
    ]
    assert "45fabc1d7bb2b1255d78a00e1daebe37ed855b7965fbae6732bf0624e426d30d" in [
        tx["txid"] for tx in transactions
    ]
    ...
    assert "c5ff30d25de95338bc4436c6fd49ce35bf74297671f9d0a5a8d52b5022623cc7" in [
        tx["txid"] for tx in transactions
    ]
    ...
    assert "0cfe0b554f766ed374a426497a9163a3beb9732e799ffa623894efaea81a4415" in [
        tx["txid"] for tx in transactions
    ]
    ...
    assert (
        "56f677b37c44f9609f6d2ee121effe5a59b0c550c9be5073d03d8c365dde0e83"
        in utxo_tx_id_list
    )
    assert (
        "39169a8dcb800afcd71adbbca93f209d5006eb787c98668e22c0550a8450f202"
        in utxo_tx_id_list
    )
    assert (
        "5f8c9b318ab9156598d3d04701c08f374832bb1ce8704e629a68961b08d290c1"
        in utxo_tx_id_list
    )
    assert (
        "a9bfc94a0a5d7e7e46f1626b9d1848da96cc85689528ca3187e55164be8f3d04"
        in utxo_tx_id_list
    )
...
async def test_async_monitoring(btc_client: AioTxBTCClient):
    ...
    assert 2811502 in blocks
    assert 2811503 in blocks
    assert "e6eda82369b59c805998fc554f55163d4e4d38f5817adf8c1b6b9f917937bd77" in [
        tx["txid"] for tx in transactions
    ]
    assert "56f677b37c44f9609f6d2ee121effe5a59b0c550c9be5073d03d8c365dde0e83" in [
        tx["txid"] for tx in transactions
